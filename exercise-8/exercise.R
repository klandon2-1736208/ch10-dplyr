...
pulitzer <- read.csv(file = "data/pulitzer-circulation-data.csv", stringsAsFactors = FALSE)
...
most_winners <- pulitzer %>%
  filter(
    Pulitzer.Prize.Winners.and.Finalists..2004.2014 == max(
...
five <- pulitzer %>%
  filter(
    ...
    # cast to an integer and take out commas
  )
